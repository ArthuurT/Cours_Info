...
err_t obj2s_detruire( obj2s_t ** liste)
{
  ...
  obj2s_t obj = **liste;
  for(j = 0; j < obj.nb; j++){        /* suppression des obj2 present dans la liste */
    obj2_detruire(&(obj.liste[j]));
  }

  free((*liste)->liste);                /* suppresion de la liste */
  (*liste)->liste = NULL;

  free(*liste);                         /* suppression de l'élèment liste (qui contient nb et la liste d'obj21) */
  (*liste) = NULL;
