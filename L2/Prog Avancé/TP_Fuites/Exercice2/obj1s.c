...
err_t obj1s_detruire( obj1s_t ** liste)
{
  ...
  obj1s_t obj = **liste;
  for(j = 0; j < obj.nb; j++){        /* suppression des obj1 present dans la liste */
    obj1_detruire(&(obj.liste[j]));
  }

free((*liste)->liste);                /* suppresion de la liste */
(*liste)->liste = NULL;

free(*liste);                         /* suppression de l'élèment liste (qui contient nb et la liste d'obj1) */
(*liste) = NULL;
