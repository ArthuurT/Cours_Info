...
unsigned long int string_comp = 0 ; 
...
extern
void string_printf( const string_t * string ) 
{
...
extern
void string_printf_cb( const void * string ) 
{
  return(string_printf( string ) ) ; 
}
...
/* 
 * Comparaison de 2 pointeurs sur des strings 
 * 1 string = string_t * 
 * 1 pointeur sur 1 string = string_t ** 
 */
...
int string_comparer_cb( const void * str1 , const void * str2 ) 
{
   ...
   * donc void * --> string_t ** 
   */
  return( string_comparer( str1 , str2 ) ) ;
}
...
extern
err_t string_fscanf( string_t ** string , 
		     FILE * restrict fd )
{
  char w_string[MAX_STRING] ;
  err_t noerr = OK ; 
  ...
  /* Lecture dans un fichier avec fscanf (pas de blancs dans la string) */
   if( fscanf(fd , "%s" , w_string ) != 1 )
     {
       printf("string_fscanf: erreur lors de la lecture de la string\n" ) ; 
       return(ERR_SCANF) ; 
     }
...
extern
err_t string_fscanf_cb( void * string , 
			FILE * restrict fd )
{
  return(string_fscanf( string , fd ) ) ; 
} 
