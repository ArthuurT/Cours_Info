...
main( int argc , char * argv[] ) 
{
  ...
  if( verbose > 1 ) avl_afficher( arbre , string_printf_cb , PREFIXE ) ; 
  ...
  for( i=0 ; i<nb_operations ; i++ ) 
    {
      ...
      if( avl_supprimer( arbre , liste_elem_lire(liste_w , i_operation ) ) )
	cpt_rech_pos++ ;
      else
	cpt_rech_neg++ ;
    }
