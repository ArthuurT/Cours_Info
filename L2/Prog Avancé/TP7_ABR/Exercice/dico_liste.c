...
main( int argc , char * argv[] ) 
{
  ...
  if( verbose ) printf( "Creation variable de travail\n" ) ;
  ...
  if( verbose ) printf( "Chargement du dictionnaire\n" ) ;
  booleen_t trace = FAUX ; 
  if( verbose ) trace = VRAI ; 
  ...
  if( verbose > 1 ) liste_afficher( liste , string_printf_cb , '\n' ) ; 
  ...
  for( i=0 ; i<nb_operations ; i++ ) 
    {
      ...
      if( liste_supprimer( liste , liste_elem_lire(liste_w , i_operation ) ) )
	cpt_rech_pos++ ;
      else
	cpt_rech_neg++ ;
    }
...
/* Statistiques */
 ...
 if( verbose ) printf( "Destruction variables de travail\n" ) ;
